...
// Material Components imports - will use reflection to avoid hard dependency
...
// Chrome Custom Tabs imports - conditionally available
 ...
 * Provides similar functionality to the iOS version with payment callbacks
 */
public class StashPayCardPlugin {
    ...
    private Object currentBottomSheetDialog; // Using Object for reflection-based access
    ...
    private Object bottomSheetBehavior; // Using Object for reflection-based access
    ...
    private class StashJavaScriptInterface {
        ...
        public void onPaymentSuccess() {
            Log.d(TAG, "Payment success received from JavaScript");
            
            // Prevent multiple calls
            if (paymentSuccessHandled) {
                Log.d(TAG, "Payment success already handled, ignoring");
                return;
            }
            ...
            new Handler(Looper.getMainLooper()).post(() -> {
                try {
                    // Show close button again since processing is complete
                    ...
                    // Auto-dismiss the dialog - call dismiss methods directly to avoid nested runOnUiThread
                    if (currentBottomSheetDialog != null) {
                        ...
                        // Check if it's the custom iOS dialog that needs animation
                        if (currentContainer != null && currentContainer.getParent() != null) {
                            // Custom iOS dialog - animate dismissal
                            dismissCustomIOSDialogDirect();
                        } else {
                            // Regular dialog
        ...
        public void onPaymentFailure() {
            Log.d(TAG, "Payment failure received from JavaScript");
        ...
        public void onPurchaseProcessing() {
            Log.d(TAG, "Purchase processing started - hiding close button");
            ...
            new Handler(Looper.getMainLooper()).post(() -> {
                if (currentCloseButton != null) {
                    ...
                    Log.d(TAG, "Close button hidden during purchase processing");
                }
                if (currentHomeButton != null) {
    ...
    public void openURL(String url) {
        ...
        // Note: initialURL will be stored in each dialog creation method AFTER cleanup
        
        // Create final variable for lambda expression
        final String finalUrl = url;
        
        Log.d(TAG, "Opening URL: " + finalUrl);
        
        activity.runOnUiThread(() -> {
            if (forceSafariViewController) {
                openWithChromeCustomTabs(finalUrl);
            } else if (useCardDrawer) {
                // Try iOS-style card first, fallback to regular dialog if it fails
                if (isMaterialComponentsAvailable()) {
                    Log.d(TAG, "Using BottomSheetDialog for iOS-style card");
                    createAndShowCardDrawer(finalUrl);
                } else {
                    Log.d(TAG, "Material Components not available, using custom iOS-style card");
                    createAndShowIOSStyleDialog(finalUrl);
                }
            } else {
    ...
    private void createAndShowDialog(String url) {
        // Aggressively clean up any existing views and dialog
        cleanupAllViews();
        
        // Store the initial URL for back button navigation (AFTER cleanup)
        initialURL = url;
        Log.d(TAG, "=== INITIAL URL STORED FOR BACK BUTTON (createAndShowDialog) ===");
        Log.d(TAG, "Stored initial URL: " + initialURL);
        ...
        try {
            ...
            // Create completely fresh container with no references to old views
            currentContainer = new FrameLayout(activity);
            setupContainerAppearance(currentContainer);
            
            // Create completely fresh WebView
            webView = new WebView(activity);
            setupWebView(webView, url);
            
            // Create completely fresh close button
        ...
        currentContainer.addView(currentHomeButton);
            ...
            Log.d(TAG, "Successfully created dialog with fresh views");
    ...
    private boolean isMaterialComponentsAvailable() {
        try {
            ...
            Log.d(TAG, "Material Components not available, falling back to regular dialog");
            return false;
        }
    }
    
    /**
     * Creates and shows a card drawer (BottomSheetDialog) - iOS-style card experience
     */
    private void createAndShowCardDrawer(String url) {
        ...
        // Store the initial URL for back button navigation (AFTER cleanup)
        initialURL = url;
        Log.d(TAG, "=== INITIAL URL STORED FOR BACK BUTTON (createAndShowCardDrawer) ===");
        Log.d(TAG, "Stored initial URL: " + initialURL);
        ...
        try {
            Log.d(TAG, "Creating iOS-style bottom sheet card");
            ...
            // Create the iOS-style card layout
            currentContainer = createIOSCardLayout();
            
            // Create drag handle area (as seen in screenshot)
            currentDragHandleArea = createIOSDragHandleArea();
            ...
            webView = new WebView(activity);
            setupIOSCardWebViewLarger(webView, url);
            currentContainer.addView(webView);
            ...
            // Create close button in top-right (iOS style)
            currentCloseButton = createIOSStyleCloseButton();
            currentControlsOverlay.addView(currentCloseButton);

            // Create Home button in top-left (iOS style), initially hidden
            if (currentHomeButton != null && currentHomeButton.getParent() != null) {
                ((ViewGroup) currentHomeButton.getParent()).removeView(currentHomeButton);
            }
            currentHomeButton = new Button(activity);
            ...
            // Configure iOS-style behavior
            configureIOSCardBehavior();
            
            // Set window properties for proper iOS appearance
            setBottomSheetWindowFlags();
            setBottomSheetBackgroundDim();
            
            Log.d(TAG, "iOS card created successfully");
            
        } catch (Exception e) {
            Log.e(TAG, "Error creating iOS card: " + e.getMessage());
            e.printStackTrace();
        ...
        // Set dismiss listener for card drawer using reflection
    ...
    /**
     * Creates iOS-style card dialog without Material Components dependency
     */
    private void createAndShowIOSStyleDialog(String url) {
        Log.d(TAG, "Creating custom iOS-style dialog");
        ...
        // Store the initial URL for back button navigation (AFTER cleanup)
        initialURL = url;
        Log.d(TAG, "=== INITIAL URL STORED FOR BACK BUTTON (createAndShowIOSStyleDialog) ===");
        Log.d(TAG, "Stored initial URL: " + initialURL);
        ...
        try {
            ...
            // Create the iOS-style card
            currentContainer = createIOSCardLayoutCustom();
            
            // Add drag handle at the top
            LinearLayout dragHandleArea = createIOSDragHandleArea();
            ...
            Log.d(TAG, "Added WebView to card container with dimensions");
        ...
        Log.d(TAG, "Card positioned with height: " + cardHeight + " pixels (68% of " + screenHeight + ")");
            ...
            isCurrentlyPresented = true;
            Log.d(TAG, "Custom iOS-style dialog created successfully");
            
        } catch (Exception e) {
            Log.e(TAG, "Error creating custom iOS-style dialog: " + e.getMessage());
            e.printStackTrace();
    ...
    /**
     * Creates the iOS-style card layout for custom dialog
     */
    private FrameLayout createIOSCardLayoutCustom() {
        ...
        Log.d(TAG, "Creating card with height: " + cardHeight + " pixels (screen height: " + screenHeight + ")");
        ...
        // Create iOS-style background
        GradientDrawable cardBackground = new GradientDrawable();
        cardBackground.setColor(Color.WHITE);
        
        // iOS corner radius - only top corners
        ...
        // Add elevation shadow for iOS effect
    ...
    private void animateSlideUp() {
        if (currentContainer != null && Build.VERSION.SDK_INT >= Build.VERSION_CODES.HONEYCOMB) {
            ...
            // Animate to position with iOS-like timing
            currentContainer.animate()
                .translationY(0)
                .setDuration(300) // iOS-like duration
    ...
    /**
     * Dismisses the custom iOS-style dialog with animation
     */
    private void dismissCustomIOSDialog() {
    ...
    /**
     * Dismisses the custom iOS-style dialog directly without runOnUiThread (for use when already on UI thread)
     */
    private void dismissCustomIOSDialogDirect() {
        if (currentDialog != null && currentContainer != null) {
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.HONEYCOMB) {
                // Get the container height for animation
                int containerHeight = currentContainer.getHeight();
    ...
    private void animateDismissCard() {
        ...
        Log.d(TAG, "Card dismissed via drag gesture");
    }
    ...
    private void animateExpandCard() {
        ...
        isExpanded = true;
        Log.d(TAG, "Card expanded to " + expandedHeight + " pixels (88% of screen)");
    }
    ...
    private void animateSnapBack() {
        ...
        Log.d(TAG, "Card snapped back to " + (isExpanded ? "expanded" : "normal") + " position");
    }
    ...
    /**
     * Sets background dimming for iOS-like overlay
     */
    private void setBottomSheetBackgroundDim() {
        try {
            if (currentBottomSheetDialog != null) {
                ...
                Window window = (Window) getWindowMethod.invoke(currentBottomSheetDialog);
                if (window != null) {
                    // Set dim amount similar to iOS (slightly less than full black)
    ...
    /**
     * Creates the iOS-style card layout matching the screenshot
     */
    private FrameLayout createIOSCardLayout() {
        FrameLayout cardContainer = new FrameLayout(activity);
        ...
        // Make the card take most of the screen like in iOS
        ...
        // Create the white card background with iOS-style corners
        GradientDrawable cardBackground = new GradientDrawable();
        cardBackground.setColor(Color.WHITE);
        
        // iOS corner radius - rounded only at top
        float cornerRadius = dpToPx(25); // Larger radius for more iOS-like appearance
        cardBackground.setCornerRadii(new float[]{
            cornerRadius, cornerRadius, // top-left
            cornerRadius, cornerRadius, // top-right  
            0, 0,                       // bottom-right (square)
        ...
        // Add proper elevation for iOS-like shadow
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
            cardContainer.setElevation(dpToPx(20)); // Strong shadow like iOS
        }
    ...
    /**
     * Creates the iOS-style drag handle area at the top of the card
     */
    private LinearLayout createIOSDragHandleArea() {
        ...
        // Padding for iOS look with semi-transparent background so it shows over WebView
        ...
        handleDrawable.setColor(Color.parseColor("#D1D1D6")); // iOS system gray 4
        handleDrawable.setCornerRadius(dpToPx(3));
        dragHandle.setBackground(handleDrawable);
        
        // iOS standard drag handle dimensions
    ...
    /**
     * Configures the BottomSheetBehavior for iOS-like card experience using reflection
     */
    private void configureIOSCardBehavior() {
        try {
            ...
            if (bottomSheet != null) {
                ...
                if (bottomSheetBehavior != null) {
                    // Configure behavior for iOS-like experience
                    ...
                    // Set half expanded ratio for iOS-like behavior
                    setBottomSheetProperty("setHalfExpandedRatio", float.class, cardHeightRatio);
                    
                    // Calculate peek height based on screen size - iOS shows substantial card content
                    DisplayMetrics metrics = activity.getResources().getDisplayMetrics();
                    int screenHeight = metrics.heightPixels;
                    
                    // iOS-style peek height - show significant content like in screenshot
                    ...
                    // Configure for iOS-like smooth behavior
                    setBottomSheetProperty("setFitToContents", boolean.class, false);
                    setBottomSheetProperty("setSkipCollapsed", boolean.class, false);
                    setBottomSheetProperty("setHideable", boolean.class, true);
                    
                    // iOS-style expansion ratios
                    setBottomSheetProperty("setHalfExpandedRatio", float.class, 0.7f);
                    setBottomSheetProperty("setExpandedOffset", int.class, dpToPx(50)); // Small gap at top like iOS
                    ...
                    // Add callback for state changes and iOS-like behavior
                ...
                (proxy, method, args) -> {
                    if ("onStateChanged".equals(method.getName()) && args.length >= 2) {
                        ...
                        if (newState == 5) { // STATE_HIDDEN = 5
                            // Auto-dismiss when hidden (like iOS drag-to-dismiss)
                            dismissBottomSheetDialog();
                        }
                        ...
                        // Similar to iOS's responsive animations during drag
                    }
                    return null;
                }
            );
    ...
    /**
     * Creates iOS-style close button for the card
     */
    private Button createIOSStyleCloseButton() {
        Button closeButton = new Button(activity);
        
        // iOS close button text and styling
        closeButton.setText("✕");
        closeButton.setTextSize(18);
        closeButton.setTextColor(Color.parseColor("#8E8E93")); // iOS gray
        ...
        // Create subtle iOS-style button background
        GradientDrawable buttonBackground = new GradientDrawable();
        buttonBackground.setColor(Color.parseColor("#F2F2F7")); // iOS system background
        buttonBackground.setCornerRadius(dpToPx(20)); // Perfect circle
        
        // Subtle border matching iOS style
        buttonBackground.setStroke(dpToPx(1), Color.parseColor("#E5E5EA"));
    ...
    /**
     * Creates iOS-style home button for returning to the initial URL
     */
    private void setupHomeButton(Button homeButton) {
        ...
        GradientDrawable buttonBackground = new GradientDrawable();
        buttonBackground.setColor(Color.parseColor("#F2F2F7"));
    ...
    private void injectPaymentCallbackScript() {
        ...
        // JavaScript to set up window.stash_sdk functions (matching iOS implementation)
    ...
    private void cleanupAllViews() {
        try {
            ...
            Log.d(TAG, "All views cleaned up successfully");
    ...
    /**
     * Sets up WebView for larger iOS-style card with explicit visibility
     */
    private void setupIOSCardWebViewLarger(WebView webView, String url) {
        ...
        webView.setWebChromeClient(new WebChromeClient() {
            ...
            public boolean onCreateWindow(WebView view, boolean isDialog, boolean isUserGesture, android.os.Message resultMsg) {
                ...
                Log.d(TAG, "WebView popup window creation requested - handling it in the same WebView");
        ...
        Log.d(TAG, "WebView height matches card height: " + webViewHeight + " pixels (68% of screen)");
        Log.d(TAG, "WebView margins: ALL ZERO - fills entire card from top to bottom");
        
        // Set clean white background to match card
        webView.setBackgroundColor(Color.WHITE);
        
        // Apply corner radius to WebView - ONLY top corners like iOS
        GradientDrawable webViewBackground = new GradientDrawable();
        webViewBackground.setColor(Color.WHITE);
        webViewBackground.setCornerRadii(new float[]{
            ...
            0, 0,                   // bottom-right corner - square (iOS style)
            0, 0                    // bottom-left corner - square (iOS style)
        });
    ...
    private void openWithReflectionChromeCustomTabs(String url) throws Exception {
        ...
        isCurrentlyPresented = true;
        Log.d(TAG, "Opened URL in Chrome Custom Tabs: " + url);
    ...
    /**
     * Sets whether to force Chrome Custom Tabs (equivalent to Safari WebView on iOS)
     */
    public void setForceSafariViewController(boolean force) {
        this.forceSafariViewController = force;
        Log.d(TAG, "ForceSafariViewController set to: " + force + " (will use " + 
              (force ? "Chrome Custom Tabs" : "WebView Dialog") + ")");
    }
    ...
    private void showCloseButton() {
        if (currentCloseButton != null && activity != null) {
            activity.runOnUiThread(() -> {
                currentCloseButton.setVisibility(View.VISIBLE);
                Log.d(TAG, "Close button shown after purchase processing");
            });
        }
    }
    ...
    private void showBackButton() {
        ...
        Log.d(TAG, "Back button disabled – showBackButton is a no-op");
        hideBackButton();
    }
    ...
    private void hideBackButton() {
        ...
        activity.runOnUiThread(() -> {
            try {
                ...
                Log.d(TAG, "Back button fully removed");
            } catch (Exception e) {
    ...
    /**
     * Shows/hides the home button based on provider domain presence
     */
    ...
    private Button createBackButton() {
        // Back button disabled – no button created
        return new Button(activity); // Will not be added/shown
    }
    ...
    private boolean shouldShowBackButton(String url) {
        if (url == null) {
            Log.d(TAG, "URL check for back button: NULL URL -> false");
            return false;
        }
        ...
        Log.d(TAG, "=== BACK BUTTON URL CHECK ===");
        Log.d(TAG, "URL: " + url);
        Log.d(TAG, "Contains klarna: " + lowerUrl.contains("klarna"));
        Log.d(TAG, "Contains paypal: " + lowerUrl.contains("paypal"));
        Log.d(TAG, "Should show back button: " + shouldShow);
